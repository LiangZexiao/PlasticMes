...
{
    public class SalaryManage_DAL
    {
        Common cmm = new Common();
        FormatSqlParas fsp = new FormatSqlParas();

        string SQL_SELECT = "SELECT ID, EmployeeName, CardID,EmpID, Do_No, MachineNo, MouldNo, ProductNo, ProductDesc, Num, BeginDate, EndDate, " +
                            " MachineWage, TimeWage, Remark, CreateDate " +
                            " FROM salarymanage " +
                            " WHERE (WageFlag =1 or WageFlag =2)  ";
        string SQL_SELECT2 = "SELECT  EmployeeName 姓名,CardID 卡号,EmpID as 工号,Do_No 派工单号,MachineNo 机器编号,MouldNo 模具编号,ProductNo 产品编号,ProductDesc 产品描述, Num 数量, BeginDate 开始时间, EndDate 结束时间," +
                            " MachineWage 计件工件, TimeWage 计时工资, AdjustWage 调整工资, OtherWage 其它工资," +
                            " CreateDate 生成时间, Remark 备注 FROM SalaryManage WHERE (WageFlag =1 or WageFlag =2) ";

        string SQL_INSERT = " INSERT INTO SalaryManage(EmployeeName, CardID, MachineNo, BeginDate, EndDate, TimeWage, Remark, CreateDate, WageFlag,EmpID ) " +
                            " VALUES(@EmployeeName, dbo.GetICCardByName(@EmployeeName), @MachineNo, @BeginDate, @EndDate,  @TimeWage, @Remark, GetDate(), 2,@EmpID)";
        ...
        string SQL_UPDATE = " UPDATE SalaryManage SET EmployeeName = @EmployeeName, CardID =  dbo.GetICCardByName(@EmployeeName), Do_No = @Do_No, MachineNo = @MachineNo, MouldNo = @MouldNo, ProductNo = @ProductNo, ProductDesc = @ProductDesc, " +
                            " BeginDate = @BeginDate, EndDate = @EndDate, MachineWage = @MachineWage, TimeWage = @TimeWage, CheckWage = @CheckWage, JobWage = @JobWage, ServiceWage = @ServiceWage, OtherWage = @OtherWage, " +
                            "  Remark = @Remark,  CreateDate = Getdate(),EmpID=@EmpID WHERE ID=@ID ";

        string SQL_DELETE = " DELETE SalaryManage WHERE ID = @ID ";

        public IList<SalaryManage_MDL> selectSalaryManage() { return null; }
        public IList<SalaryManage_MDL> selectSalaryManage(int id) { return null; }
        public IList<SalaryManage_MDL> selectSalaryManage(int id, string colname, string coltext)
        {
            ...
            IList<SalaryManage_MDL> SalaryManageinfoList = new List<SalaryManage_MDL>();
            using (SqlDataReader sdr = SqlHelper.ExecuteReader(CommandType.Text, sqlCmd.Append(" ORDER BY BeginDate DESC").ToString(), sqlParas))
            {
                while (sdr.Read())
                {
                    SalaryManage_MDL SalaryManageinfo = new SalaryManage_MDL(
                        ...
                        (sdr["Num"] == DBNull.Value) ? "" : sdr["Num"].ToString(),                       
        ...
        public IList<SalaryManage_MDL> selectSalaryManage(int id, string colname, string coltext, string BeginDate, string EndDate)
        {
            StringBuilder sqlCmd = new StringBuilder(SQL_SELECT);
            SqlParameter[] sqlParas = null;
            ...
            {
                sqlCmd.Append(string.Format(@"AND ((CONVERT(CHAR(16),BeginDate,121)>='{0}' AND  CONVERT(CHAR(16),BeginDate,121)<='{1}')
                                      or (convert(char(16),EndDate,121) >='{0}' AND  CONVERT(CHAR(16),EndDate,121)<='{1}'))
                                ", BeginDate, EndDate));
                sqlParas = null;
            }
            //注塑车间植毛车间单独统计
        ...
        public IList<SalaryManage_MDL> existsSalaryManage(string _EmployeeName)
        {
            StringBuilder sqlCmd = new StringBuilder("SELECT DISTINCT DO_DO FROM SalaryManage WHERE EmployeeName=@EmployeeName");
            SqlParameter[] sqlParas = { fsp.FormatInParam("@EmployeeName", SqlDbType.VarChar, _EmployeeName) };
            ...
            using (SqlDataReader sdr = SqlHelper.ExecuteReader(CommandType.Text, sqlCmd.ToString(), sqlParas))
            {
                while (sdr.Read())
                {
                    SalaryManage_MDL SalaryManageinfo = new SalaryManage_MDL((sdr["EmployeeName"] == DBNull.Value) ? "" : sdr["EmployeeName"].ToString());
                    SalaryManageinfoList.Add(SalaryManageinfo);
                }
            }
            return SalaryManageinfoList;
        }
        public IList<SalaryManage_MDL> existsSalaryManage(string colname, string coltext)
        {
        ...
        public int ChangeSalaryManage(SalaryManage_MDL obj, string IU)
        {
        ...
        {
            SqlParameter[] sqlParas = { fsp.FormatInParam("@BeginDate", SqlDbType.VarChar, beginDate), fsp.FormatInParam("@EndDate", SqlDbType.VarChar, EndDate) };

            IList<SalaryManage_MDL> SalaryManagelist = new List<SalaryManage_MDL>();
